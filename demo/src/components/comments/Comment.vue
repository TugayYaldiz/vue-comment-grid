  ...
  export default {
    ...
    methods: {
      ...
      reply() {
        ...
        if (this.auth) {
          if (this.filterNewComment.length != 0) {
            ...
            let replyObj = { name: this.filterUserName, comment: this.filterNewComment, user_id: this.userId, timestamp: Date.now().toString(), lineCount: this.filterNewCommentLineCount }
            axios.post(this.baseURL + '/commentsGrid/' + this.nodeName + '/replys/' + this.id + '.json' + '?auth=' + this.idToken, replyObj)
              .then(res => {
                ...
                let repliedObj = { replied: false, user_id: this.userId, mods: obj }
                axios.put(this.baseURL + '/commentsGrid/' + this.nodeName + '/replys/' + replyObj.id + '.json' + '?auth=' + this.idToken, repliedObj)
                  .then(res => {
                    replyObj.depth = 'commentsGrid/' + this.nodeName + '/replys/' + this.id + '/' + replyObj.id
        ...
        if (this.auth) {
          ...
          if (!this.isAdmin) {
            if(this.filterNewUpdate.length!=0) {
              let replyObj = { name: this.filterUserName, comment: this.filterNewUpdate, user_id: this.userId, lineCount: this.filterNewUpdateLineCount }
              console.log(this.depth);
              axios.patch(this.baseURL + '/' + this.depth + '.json' + '?auth=' + this.idToken, replyObj)
                .then(res => {
                  this.clearAlert()
      ...
      deleteComment() {
        ...
        axios.delete(this.baseURL + '/' + changeDept1 + '.json' + '?auth=' + this.idToken)
          .catch(err => console.log(err))
        const changedDepth2 = this.depth.replace('/comments/', '/replys/')
        axios.get(this.baseURL + '/' + changedDepth2 + '.json')
          .then(res => {
            const data = res.data;
            for (let key in data) {
              if (key.startsWith("-")) {
                axios.delete(this.baseURL + '/commentsGrid/' + this.nodeName + '/replys/' + key + '.json' + '?auth=' + this.idToken)
                  .catch(err => console.log(err))
              }
            }
          })
          .catch(err => console.log(err))
        axios.delete(this.baseURL + '/' + changedDepth2 + '.json' + '?auth=' + this.idToken)
          .then(res => {
            this.replys = []
            this.$emit('delete-row');
          })
    ...
    watch: {
      ...
      maxShowingDepth() { // this is for demo
    ...
    created() {
      ...
      axios.get(this.baseURL + '/commentsGrid/' + this.nodeName + '/replys/' + this.id + '.json')
        .then(res => {
          const data = res.data;
          for (let key in data) {
            if (key.startsWith("-")) {
              let reply = data[key]
              reply.id = key
              reply.depth = 'commentsGrid/' + this.nodeName + '/replys/' + this.id + '/' + reply.id
    ...
    mounted() {
      this.$nextTick(() => {
        for (let i = 0; i < 10; i++) {
          this.$refs.svgAvatar.innerHTML += "<circle cx=" + (Math.floor(Math.random() * (35 - 5)) + 5) + " cy=" + (Math.floor(Math.random() * (35 - 5)) + 5) + " r='" + Math.round(Math.random() * 15) + "' fill='rgba(" + Math.round(Math.random() * 244) + "," + Math.round(Math.random() * 244) + "," + Math.round(Math.random() * 244) + "," + Math.random() + ")'> </circle>";
        }
        let updateHeight = this.$refs.comment.clientHeight
        if (updateHeight != 0) {
